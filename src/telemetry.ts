...
export async function funnel(days: number) {
  ...
  const [visits, docs, referrers, mcp, mcpClients, tools, errors, joins, participants, members] = await Promise.all([
    pool.query(`select at::date as day, ua_class, count(*)::int as hits, count(distinct ip_hash)::int as uniques from web_visits where coalesce(user_agent,'') not like '%agent-ecology%' and at > now() - ($1::int * interval '1 day') group by 1,2 order by 1 desc`, [d]),
    pool.query(`select path, count(*)::int as hits, count(distinct ip_hash)::int as uniques from web_visits where coalesce(user_agent,'') not like '%agent-ecology%' and at > now() - ($1::int * interval '1 day') and path in ('/llms.txt','/protocol.md','/for-agents','/protocol','/.well-known/agent-card.json','/.well-known/mcp/server-card.json') group by 1 order by hits desc`, [d]),
    pool.query(`select referrer_host, count(*)::int as hits from web_visits where coalesce(user_agent,'') not like '%agent-ecology%' and at > now() - ($1::int * interval '1 day') and referrer_host is not null group by 1 order by hits desc limit 15`, [d]),
    pool.query(`select at::date as day, kind, count(*)::int as n, count(distinct ip_hash)::int as uniques from mcp_events where coalesce(user_agent,'') not like '%agent-ecology%' and at > now() - ($1::int * interval '1 day') group by 1,2 order by 1 desc`, [d]),
    pool.query(`select coalesce(client_name,'(unnamed)') as client, count(*)::int as n, count(distinct ip_hash)::int as uniques, max(at) as last_seen from mcp_events where coalesce(user_agent,'') not like '%agent-ecology%' and kind='initialize' and at > now() - ($1::int * interval '1 day') group by 1 order by n desc`, [d]),
